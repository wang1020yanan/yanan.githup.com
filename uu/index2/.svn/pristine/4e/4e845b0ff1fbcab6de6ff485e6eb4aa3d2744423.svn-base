...
<SCRIPT language=javascript>
	...
	{foreach key=key value=templates from=$_debug_tpls}
		...
		_templatelite_console.document.write("<fonts color={if $_debug_tpls[$key].type eq "template"}brown{elseif $_debug_tpls[$key].type eq "insert"}black{else}green{/if}>");
		_templatelite_console.document.write("{$_debug_tpls[$key].filename}</fonts>{if isset($_debug_tpls[$key].exec_time)} ");
		_templatelite_console.document.write("<fonts size=-1><i>({$_debug_tpls[$key].exec_time|string_format:"%.5f"} seconds){if $key eq 0} (total){/if}");
	...
	{foreach key=key value=vars from=$_debug_keys}
		_templatelite_console.document.write("<tr bgcolor={if $key % 2}#eeeeee{else}#fafafa{/if}>");
		_templatelite_console.document.write("<td valign=top><tt><fonts color=blue>{ldelim}${$_debug_keys[$key]}{rdelim}</fonts></tt></td>");
		_templatelite_console.document.write("<td nowrap><tt><fonts color=green>{$_debug_vals[$key]|@debug_print_var}</fonts></tt></td></tr>");
	{foreachelse}
	...
	{foreach key=key value=config_vars from=$_debug_config_keys}
		_templatelite_console.document.write("<tr bgcolor={if $key % 2}#eeeeee{else}#fafafa{/if}>");
		_templatelite_console.document.write("<td valign=top><tt><fonts color=maroon>{ldelim}#{$_debug_config_keys[$key]}#{rdelim}</fonts></tt></td>");
		_templatelite_console.document.write("<td><tt><fonts color=green>{$_debug_config_vals[$key]|@debug_print_var}</fonts></tt></td></tr>");
	{foreachelse}
